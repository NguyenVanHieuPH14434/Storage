...
const Producer = () => {
  ...
  //call api mỗi khi sử dụng button
  useEffect(() => {
...
// lấy dữ liêu ở phần Create
  ...
  // create
  const handleCreate = () => {
    let { producer_name, producer_address, producer_email, producer_phone } =
      infoProducer;
    ...
    if (
      ...
      if (producer_phone !== "") {
        if (vnf_regex.test(producer_phone) === false) {
          ...
          axios
            .post(
              `http://localhost:4000/api/producer/update/${infoProducer._id}`,
              infoProducer
            )
            .then((res) => {
              setInfoProducer({
